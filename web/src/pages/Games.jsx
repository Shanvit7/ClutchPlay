...
const Games = () => {
  ...
  const { data: yesterdaysGame = {}, status: yesterdaysGameStatus } = useQuery(
    "yesterDaysGame",
    () =>
      getGamesByDate(
        formattedDate(yesterdaysDate),
        formattedDate(yesterdaysDate)
      )
  );
  ...
  return (
    <div className="w-screen h-screen bg-black overflow-x-hidden">
      <SideBarWrapper>
        ...
        <GameAlert />
        <GameSection
          games={todaysGame.data}
          status={todaysGameStatus}
          title={`Today's Game`}
        />
        <GameSection
          games={tommorrowsGame.data}
          status={tommorrowsGameStatus}
          title={`Scheduled Tommorrow`}
        />
        <GameSection
          games={yesterdaysGame.data}
          status={yesterdaysGameStatus}
